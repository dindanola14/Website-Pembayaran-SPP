...
// Jika formulir dikirimkan, masukkan nilai ke dalam database
    if (isset($_POST['username'])){
        ...
        $query = "SELECT * FROM petugas WHERE username='$username' and level='petugas' and password='".($password)."'";
        $result = mysqli_query($con,$query) or die(mysql_error());
        $rows = mysqli_num_rows($result);
            if($rows==1){
                $_SESSION['level'] == 'petugas';
                $_SESSION['username'] = $username;
                header("Location:admin/indexpetugas.php"); // Dilarikan ke indexpetugas.php
            }else{
